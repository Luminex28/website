export function ProjectVisual({ variant }: { variant: "waveform" | "process" }) {
  if (variant === "waveform") {
    // Lumin — a launcher's query/response rhythm, rendered as signal bars
    // Round to 2dp — SSR/client libm implementations of sin can differ in
  ...
  // TaskL — layered system panes, core vs. UI separation
  return (
    <svg viewBox="0 0 960 400" preserveAspectRatio="none" aria-hidden="true">
      <rect width="960" height="400" fill="var(--surface)" />
      ...
      {Array.from({ length: 8 }, (_, i) => (
        <rect key={i} x="64" y={72 + i * 34} width={220 - (i % 3) * 40} height="10" fill="var(--line-strong)" />
      ))}
      <circle cx="640" cy="115" r="46" fill="none" stroke="var(--accent)" strokeWidth="2" opacity="0.85" />
      <path d="M640 69 A46 46 0 0 1 686 115" fill="none" stroke="var(--depth)" strokeWidth="2" />
      {Array.from({ length: 5 }, (_, i) => (
        <rect key={i} x="400" y={255 + i * 20} width={480 - i * 60} height="7" fill="var(--line-strong)" />
      ))}
    </svg>
  );
}
